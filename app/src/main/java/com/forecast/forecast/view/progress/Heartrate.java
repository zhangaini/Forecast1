 ...
 */
public class Heartrate extends View {
    ...
    private static class MyHandler extends Handler {
        private WeakReference<Heartrate> mWeakRef = null;

        private int refreshPeriod = 100;

        public MyHandler(Heartrate host) {
            mWeakRef = new WeakReference<Heartrate>(host);
        }
    ...
    public Heartrate(Context paramContext) {
        super(paramContext);
    }

    public Heartrate(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0, mCount);
    }

    public Heartrate(Context context, AttributeSet attrs,
                             int defStyleAttr, int count) {
        super(context, attrs, defStyleAttr);
        attrInit = new WaterWaveAttrInit(context, attrs, defStyleAttr, count);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (getProgress() > 100) {
            mRingColor = getmRingColor();
            mRingBgColor = getmRingBgColor();
            mWaterColor = getWaterColor();
            mWaterBgColor = getWaterBgColor();
        }
        // 画进度条背景
        mRingPaint.setColor(mRingBgColor);
        // canvas.drawArc(oval, -90, 360, false, mRingPaint);
