...
import com.forecast.forecast.adapters.wheel.ArrayWheelAdapter;
import com.forecast.forecast.intefaces.OnItemSelectedListener;
import com.forecast.forecast.intefaces.WheelOption1Listener;
import com.forecast.forecast.intefaces.WheelOption2Listener;
...
public class WheelOptions<T> {
	...
	//省滚轴监听
	private WheelOption1Listener option1Listener1;
	//市滚轴监听
	private WheelOption2Listener option2Listener;
	...
	public WheelOptions(View view, WheelOption1Listener option1Listener1, WheelOption2Listener option2Listener) {
		super();
		this.view = view;
		setView(view);
		this.option1Listener1 = option1Listener1;
		this.option2Listener = option2Listener;
	}
	...
	public void setPicker(ArrayList<T> options1Items,
		...
		ArrayWheelAdapter option1Adapter = new ArrayWheelAdapter(mOptions1Items, len);
		wv_option1.setAdapter(option1Adapter);// 设置显示数据
		wv_option1.setCurrentItem(0);// 初始化时显示的数据
		// 选项2
		wv_option2 = (WheelView) view.findViewById(R.id.options2);
		if (mOptions2Items != null)
			wv_option2.setAdapter(new ArrayWheelAdapter(mOptions2Items.get(options2Position)));// 设置显示数据
		wv_option2.setCurrentItem(wv_option1.getCurrentItem());// 初始化时显示的数据
		// 选项3
		wv_option3 = (WheelView) view.findViewById(R.id.options3);
		if (mOptions3Items != null)
			wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items.get(options2Position)
					.get(options3Position)));// 设置显示数据
        ...
        wheelListener_option1 = new OnItemSelectedListener() {
			...
			public void onItemSelected(int index) {
				//监听当前的position的长度
				option1Listener1.onItemSelected(mOptions2Items.get(index).size(), index);
				int opt2Select = 0;
				if (mOptions2Items != null) {
				...
				if (mOptions3Items != null) {
					...
					if (index ==-1 ){
						...
						if (mOptions3Items.size() < opt1Select){
							...
							option2Listener.onItemSelected( mOptions3Items.get(opt1Select).get(index).size(), index);
						}
